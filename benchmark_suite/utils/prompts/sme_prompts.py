...
def create_prompt(question: str, num_prompts: int = 8, method: str = 'all', seed: int = 42) -> str:
    """
    Create a prompt for SME task with few-shot examples.
    
    Args:
        question: The question to be answered
        num_prompts: Number of examples to include when using random method
        method: Either 'all' or 'random' for example selection
        seed: Random seed for reproducibility
        
    Returns:
        str: Formatted prompt with examples and the question
    """
    if method == 'all':
        prompt_examples = SME_CURATED_EXAMPLES
    elif method == 'random':
        random.seed(seed)
        prompt_examples = random.sample(SME_CURATED_EXAMPLES, num_prompts)
    else:
        raise NotImplementedError("Only 'all' and 'random' methods are supported")
